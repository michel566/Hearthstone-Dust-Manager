...
object UIUtil {
    ...
    fun selectionOptionList(
        context: Context?, title: String?,
        onClick: DialogInterface.OnClickListener?, vararg options: String?
    ): AlertDialog.Builder {
        val builder = AlertDialog.Builder(context)
    ...
    @JvmStatic
    fun setTextViewToTooltipDialog(
        context: Context,
        textsToDialog: List<String?>,
        textViewers: List<TextView>
    ) {
        var index = 0
        for (tv in textViewers) {
            val finalIndex = index
            tv.setOnLongClickListener {
                val tooltipDialog = LightDialog(
                    context,
                    textsToDialog[finalIndex], DialogType.INFO, true
                )
                tooltipDialog.show()
                tooltipDialog.onOptionConfirmClickListener(
                    { tooltipDialog.dismiss() },
                    context.resources.getString(R.string.dialog_bt_ok)
                )
                true
            }
            index++
